...
def load_valid_file(valid_file):
    return torch.load(valid_file)
...
def filter_bad_images(is_valid):
...
def get_data(train_data_path, batch_size, preprocess_fns, epoch=0, tokenizer=None, valid_file=None, train_samples=0):
    preprocess_train, preprocess_val = preprocess_fns
    data = {}
    data["train"] = get_wds_dataset(
        train_data_path, batch_size, preprocess_train, is_train=True, epoch=epoch, tokenizer=tokenizer, valid_file=valid_file, train_samples=train_samples)
